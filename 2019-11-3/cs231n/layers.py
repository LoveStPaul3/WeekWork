...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    #首先输入为k+1维，下次层为M维，所以输出out应该为N*M维
    N = x.shape[0]
    x_reshape = x.reshape([N, -1])#这里其实是一个转置操作，shape[0]就是第一个列的行数，此时-1表示列数待定，最终形成二维的矩阵
...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    #这里有一个小技巧，什么时候应该转置？什么时候应该求和?  
    #最好的方法应该是根据各个变量的shape来确定
    #例如对于dx，通过上面前向计算表达式可知其导数为dout * w，由于dout.shape == (N, M)，w.shape == (D, M)，而dx.shape==(N, d1, ..., d_k)
    N = x.shape[0]
